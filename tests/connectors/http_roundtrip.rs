...
#[test]
fn collect_record_roundtrip_produces_kafka_publish_and_http_response() -> TestResult {
    ...
    let trigger_frame = execution.context[0]
        .as_object()
        .ok_or("trigger frame must be an object")?;
    assert_eq!(
        trigger_frame
            .get("headers")
    ...
    let summary_frame = execution.context[1]
        .as_object()
        .ok_or("summary frame must be an object")?;
    assert_eq!(
        summary_frame
            .get("summary")
    ...
    let response_frame = execution.context[3]
        .as_object()
        .ok_or("response frame must be an object")?;
    assert_eq!(
        response_frame
            .get("body")
    ...
    let ChronicleAction::KafkaPublish {
        connector,
        topic,
        payload,
        trace_id,
    ...
    let response = execution.response.ok_or("http response missing")?;
...
#[test]
fn http_response_encoding_serialises_structured_bodies() -> TestResult {
    ...
    let decoded: serde_json::Value = serde_json::from_slice(&encoded.body)?;
    assert_eq!(

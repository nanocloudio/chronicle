...
#[test]
fn integration_config_matches_spec_schema() -> TestResult {
    ...
    let grpc_connector = config
        ...
        .ok_or("grpc connector present")?;
    let grpc_options_json = grpc_connector.options_json();
    let grpc_options = grpc_options_json.as_object().ok_or("grpc options object")?;
    assert_eq!(
    ...
    let smtp_connector = config
        ...
        .ok_or("smtp connector present")?;
    let smtp_options_json = smtp_connector.options_json();
    let smtp_options = smtp_options_json.as_object().ok_or("smtp options object")?;
    ...
    let collect_record = config
        ...
        .ok_or("collect_record chronicle present")?;
    ...
    let relay_record = config
        ...
        .ok_or("relay_record chronicle present")?;
    let relay_trigger = relay_record
        .trigger
        .kafka_json()
        .and_then(|json| json.as_object().cloned())
        .ok_or("relay_record uses kafka trigger")?;
...
#[test]
fn app_section_rejects_unknown_keys() -> TestResult {
    let yaml = r#"
app:
  warmup_timeout: 10s
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("unknown app key should fail validation")?;
...
#[test]
fn connector_section_rejects_unknown_keys() -> TestResult {
    let yaml = r#"
connectors:
  - name: inbound
    type: http_server
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("unknown connector key should fail validation")?;
...
#[test]
fn chronicle_section_rejects_unknown_keys() -> TestResult {
    let yaml = r#"
connectors:
  - name: inbound
    type: http_server
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("unknown chronicle key should fail validation")?;
...
#[test]
fn management_section_rejects_unknown_keys() -> TestResult {
    let yaml = r#"
management:
  host: 0.0.0.0
  port: 9100
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("unknown management key should fail validation")?;
...
#[test]
fn delivery_retries_cannot_exceed_budget() -> TestResult {
    let yaml = r#"
api_version: v1
app:
  retry_budget:
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("delivery retries above retry budget should fail")?;
...
#[test]
fn app_warmup_timeout_must_be_positive() -> TestResult {
    let yaml = r#"
api_version: v1
app:
  warmup_timeout: 0s
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("zero warmup timeout should fail")?;
...
#[test]
fn app_retry_budget_durations_must_be_positive() -> TestResult {
    let yaml = r#"
api_version: v1
app:
  retry_budget:
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("invalid retry budget should fail")?;
...
#[test]
fn connector_warmup_timeout_must_be_positive() -> TestResult {
    let yaml = r#"
api_version: v1
connectors:
  - name: inbound
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("connector warmup timeout should fail")?;
...
#[test]
fn rabbitmq_connector_requires_feature_flag() -> TestResult {
    let yaml = r#"
api_version: v1
connectors:
  - name: rabbit
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("rabbitmq connector should require flag")?;
...
#[test]
fn parallel_phase_requires_feature_flag() -> TestResult {
    let yaml = r#"
api_version: v1
connectors:
  - name: http_ingest
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("parallel phases should require flag")?;
...
#[test]
fn unknown_feature_flags_fail_validation() -> TestResult {
    let yaml = r#"
api_version: v1
app:
  feature_flags:
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("unknown feature flag should fail validation")?;
...
#[test]
fn queue_overflow_requires_depth_and_positive_limits() -> TestResult {
    let yaml = r#"
api_version: v1
app:
  limits:
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("queue overflow without depth should fail")?;
...
#[test]
fn chronicle_policy_limit_errors_reference_chronicle() -> TestResult {
    let yaml = r#"
api_version: v1
connectors:
  - name: inbound
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("chronicle policy limits should be validated")?;
...
#[test]
fn http_trigger_requires_server_connector_role() -> TestResult {
    let yaml = r#"
api_version: v1
connectors:
  - name: outbound
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("http trigger should reject client connectors")?;
...
#[test]
fn connector_circuit_breaker_validation_runs() -> TestResult {
    let yaml = r#"
api_version: v1
connectors:
  - name: api
...
"#;
    ...
    let error = IntegrationConfig::from_reader(yaml.as_bytes())
        .err()
        .ok_or("invalid circuit breaker should fail")?;
